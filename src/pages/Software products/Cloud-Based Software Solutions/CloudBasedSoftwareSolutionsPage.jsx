...
import { CLOUD_BASED_SOFTWARE_SPREADS } from './CloudBasedSoftwarePageData';
...
const GoldCorner = ({ position = 'top-left' }) => {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="gold-grad-cld" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#FFEAA7" />
          <stop offset="35%" stopColor="#F59E0B" />
          <stop offset="70%" stopColor="#D97706" />
          <stop offset="100%" stopColor="#78350F" />
        </linearGradient>
        <filter id="gold-glow-cld" x="-10%" y="-10%" width="120%" height="120%">
      ...
      <path
        ...
        fill="url(#gold-grad-cld)"
        stroke="#78350F"
        strokeWidth="1.5"
        filter="url(#gold-glow-cld)"
      />
      <path
...
export default function CloudBasedSoftwareSolutionsPage({ onBack }) {
  const title = "Cloud-Based Software Solutions";
  const subtitle = "Flexible Computing Platforms & Distributed Digital Infrastructure — CR Cyber Crime Foundation";
  const spreads = CLOUD_BASED_SOFTWARE_SPREADS;
  ...
  useEffect(() => {
    if (!window.history.state?.cloudReaderOpen) {
      window.history.pushState({ cloudReaderOpen: true }, '');
    }

    const handlePopState = (event) => {
      const state = event.state;
      if (!state || !state.cloudReaderOpen) {
        if (onBack) onBack();
      }
    };
    ...
    return (
      <div className={`w-full h-full pt-3.5 pb-3 sm:pt-4.5 sm:pb-4 ${sidePaddingClass} flex flex-col justify-between select-none backface-hidden absolute inset-0 bg-transparent`}>
        ...
        <div className="shrink-0 flex items-center justify-between pt-1 mt-1 border-t border-[#8a5a36]/25 text-[9.5px] sm:text-[10.5px] text-[#8a5a36] font-mono">
          <span>{side === 'left' ? 'CR CYBER CRIME FOUNDATION' : 'CLOUD-BASED SOFTWARE SOLUTIONS'}</span>
