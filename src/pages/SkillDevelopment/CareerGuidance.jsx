...
import { 
  ...
  Handshake, LifeBuoy, FileText, CheckCircle2,
  BookOpen, Lightbulb, Compass, Award, Building2
} from "lucide-react";
...
// ─── Extracted sub-component so hooks are not called inside .map() ───────────
const ScrollCard = memo(({ section, index, totalSections, scrollYProgress }) => {
  ...
  const yRaw      = useTransform(scrollYProgress, [start, focus, exit], ["120%", "0%", "-100%"]);
  const scaleRaw  = useTransform(scrollYProgress, [start, focus, exit], [0.7, 1, 0.85]);
  const opacityRaw= useTransform(scrollYProgress, [start, focus, exit], [0, 1, 0]);
...
ScrollCard.displayName = "CareerScrollCard";
// ─────────────────────────────────────────────────────────────────────────────

const FloatingIconsBackground = memo(() => {
  const icons = [
      ...
      {icons.map((item, i) => {
        ...
        return (
          <motion.div
            ...
            style={{ top, left, right, willChange: "transform" }}
...
FloatingIconsBackground.displayName = "CareerFloatingBg";
