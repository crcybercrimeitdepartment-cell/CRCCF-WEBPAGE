...
const inputClasses = "w-full bg-white border border-emerald-200 rounded-xl px-4 py-3 text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all shadow-sm placeholder-gray-400 text-gray-700";
const labelClasses = "block text-xs font-bold text-emerald-800 uppercase tracking-wider mb-2 ml-1";
const sectionClasses = "bg-white/60 backdrop-blur-md rounded-2xl p-5 sm:p-6 md:p-8 shadow-sm border border-emerald-100";
const sectionTitleClasses = "text-lg sm:text-xl font-bold text-emerald-900 mb-6 flex items-center gap-3 border-b border-emerald-200 pb-4";
  ...
  <div>
    ...
    <div className="relative">
      <input
        ...
        className={`${inputClasses} ${error ? 'border-emerald-400 ring-2 ring-emerald-100' : ''} ${Icon ? 'pl-11' : ''}`}
      />
      {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />}
    </div>
    {error && <p className="text-emerald-500 text-xs mt-1 flex items-center gap-1 font-semibold">⚠ {error}</p>}
  </div>
);
  ...
  <div>
    ...
    <div className="relative">
      <textarea
        ...
        placeholder={placeholder || `Enter ${label}`}
        className={`${inputClasses} resize-none ${error ? 'border-emerald-400 ring-2 ring-emerald-100' : ''} ${Icon ? 'pl-11 pt-4' : ''}`}
      />
    ...
    <div>
      ...
      <div className="relative">
        ...
        <div 
          ...
          className={`w-full bg-white border ${isOpen ? 'border-emerald-500 ring-4 ring-emerald-50' : 'border-emerald-200'} rounded-xl px-4 py-3 text-base sm:text-sm transition-all shadow-sm flex justify-between items-center cursor-pointer hover:border-emerald-300 ${error ? 'border-emerald-400 ring-2 ring-emerald-100' : ''} ${Icon ? 'pl-11' : ''}`}
        >
          {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />}
          ...
          <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
            <FiChevronDown className={`w-4 h-4 ${isOpen ? 'text-emerald-500' : 'text-gray-400'}`} />
          </motion.div>
        </div>
          ...
          {isOpen && (
            <>
              ...
              <motion.div
                ...
                className="absolute z-50 w-full mt-2 bg-white border border-emerald-100 rounded-xl shadow-[0_10px_40px_-10px_rgba(16,185,129,0.2)] overflow-hidden origin-top"
              >
                <div className="max-h-60 overflow-y-auto py-1 custom-scrollbar">
                  {options.map((option) => (
                    <div
                      ...
                      className={`px-4 py-3 sm:py-2.5 text-base sm:text-sm cursor-pointer transition-colors flex items-center justify-between ${value === option.value ? 'bg-emerald-50 text-emerald-700 font-bold' : 'text-gray-700 hover:bg-emerald-50/50'}`}
                    >
                      {option.label}
                      {value === option.value && <FiCheckCircle className="w-4 h-4 text-emerald-500" />}
                    </div>
                  ))}
                </div>
              </motion.div>
...
const STEPS = [
  ...
  { label: 'School Details',      Icon: MdSchool },
];

export default function ITICertificateForm({ onBack }) {
  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      ...
      if (!formData.dateOfBirth) e.dateOfBirth = 'Date of birth is required.';
        ...
        else if (age < 10) e.dateOfBirth = 'Age must be at least 10 years.';
        else if (age > 40) e.dateOfBirth = 'Age seems too high (max 40 years for ITI student).';
      }
    ...
    if (step === 1) {
      ...
      if (!formData.nameOfBoard.trim()) e.nameOfBoard = 'Board name is required.';
  ...
  if (isPreview) {
    return (
      <div className="min-h-screen bg-[#ecfdf5] relative overflow-hidden font-sans pb-20 pt-28 sm:pt-32">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="bg-white/95 backdrop-blur-md rounded-3xl p-6 sm:p-10 shadow-xl border border-emerald-100">
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-50 text-emerald-700 text-xs font-bold mb-3 border border-emerald-200">
            ...
            <div className="bg-slate-50/80 rounded-2xl p-6 border border-slate-200 mb-8 space-y-6">
              <div>
                <h3 className="text-xs font-black uppercase tracking-wider text-emerald-800 mb-3 pb-2 border-b border-slate-200">
            ...
            <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-200">
              ...
              <button
                ...
                className="w-full sm:w-auto px-8 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 shadow-lg shadow-emerald-300/50 transition-all hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer"
              >
  ...
  if (submitted) {
    return (
      ...
      >
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center border border-emerald-100">
          <motion.div 
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          <p className="text-gray-500 mb-8">Your ITI Certificate Application has been successfully submitted.</p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
            <button onClick={handleReset} className="text-emerald-600 font-bold hover:underline">Submit Another Application</button>
  ...
  return (
    <div className="min-h-screen bg-[#ecfdf5] relative overflow-hidden font-sans pb-20">
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-emerald-200/50 rounded-full mix-blend-multiply filter blur-[80px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-teal-200/50 rounded-full mix-blend-multiply filter blur-[80px] pointer-events-none" />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-28 sm:pt-32 pb-20 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex flex-col items-center text-center">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-5 mb-4">
            <div className="inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 shadow-lg shadow-emerald-300/50 text-white shrink-0">
              <MdSchool className="w-7 h-7 sm:w-8 sm:h-8" />
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight text-center sm:text-left">
              ITI Certificate <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-500">Application Form</span>
            </h1>
          </div>
          <p className="text-gray-600 max-w-2xl text-base sm:text-lg mx-auto">
            Please fill out all the required details carefully to request your ITI academic certificate.
          </p>
        </motion.div>
        ...
        <div className="flex justify-end max-w-3xl mx-auto mb-3 px-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-emerald-300 bg-emerald-50/90 text-emerald-900 hover:bg-emerald-100 font-bold text-xs shadow-xs transition-all cursor-pointer"
          >
            <FiFastForward className="w-3.5 h-3.5 text-emerald-600" />
            <span>Skip to Preview</span>
          </button>
        </div>
<div className="mb-12 max-w-3xl mx-auto px-4 sm:px-0">
          <div className="flex items-center justify-between relative">
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1.5 bg-emerald-100 -z-10 rounded-full"></div>
            <div 
              className="absolute left-0 top-1/2 -translate-y-1/2 h-1.5 bg-gradient-to-r from-emerald-400 to-emerald-600 -z-10 rounded-full transition-all duration-500 ease-in-out"
            ...
            {STEPS.map((stepData, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center">
                  <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all duration-500 border-4 ${isActive ? 'bg-emerald-600 border-emerald-100 text-white shadow-lg shadow-emerald-300' : 'bg-white border-emerald-100 text-emerald-300'}`}>
                    <StepIcon className="w-4 h-4 sm:w-6 sm:h-6" />
                  </div>
                  <span className={`mt-3 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-center ${isActive ? 'text-emerald-800' : 'text-emerald-400'} hidden sm:block`}>{stepData.label}</span>
                </div>
              )
            })}
          </div>
          <div className="text-center mt-4 sm:hidden">
             <span className="text-sm font-bold text-emerald-800 uppercase tracking-wider">{STEPS[step].label}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="relative">
            {step === 0 && (
              <motion.div key="step0" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-emerald-100 rounded-lg text-emerald-600"><FiUser className="w-5 h-5" /></div>
                  Personal Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                  ...
                  <div className="sm:col-span-2 lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                    ...
                    <div>
                      ...
                      <label className="flex items-center gap-1.5 cursor-pointer text-[10px] sm:text-xs font-bold text-emerald-700 hover:text-emerald-800 transition-colors uppercase tracking-wider mt-2">
                        <input type="checkbox" checked={sameAddress} onChange={handleSameAddress} className="w-4 h-4 text-emerald-600 rounded border-emerald-300 focus:ring-emerald-500 cursor-pointer accent-emerald-600" />
            ...
            {step === 1 && (
              <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-emerald-100 rounded-lg text-emerald-600"><FiFileText className="w-5 h-5" /></div>
                  Certificate Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                  ...
                  <div><SelectField label="Year of Passing (ITI)" id="yearOfPassing" value={formData.yearOfPassing} onChange={handleChange} required icon={MdOutlineDateRange} options={years} error={errors.yearOfPassing} /></div>
                  <div><InputField label="Name of the Board" id="nameOfBoard" value={formData.nameOfBoard} onChange={handleChange} placeholder="e.g. CBSE, State Board" required icon={FiBriefcase} error={errors.nameOfBoard} /></div>
                  <div>
                    <SelectField label="Type of Board" id="typeOfBoard" value={formData.typeOfBoard} onChange={handleChange} required icon={FiList} error={errors.typeOfBoard}
                      options={[{ value: 'state', label: 'State' }, { value: 'cbse', label: 'CBSE' }, { value: 'icse', label: 'ICSE' }, { value: 'others', label: 'Others' }]} />
                  </div>
            ...
            {step === 2 && (
              <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-emerald-100 rounded-lg text-emerald-600"><FiBarChart2 className="w-5 h-5" /></div>
                  Academic Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                  ...
                  <div className="sm:col-span-2 lg:col-span-3">
                    <TextAreaField label="Subjects Studied (Main Subjects)" id="subjectsStudied" value={formData.subjectsStudied} onChange={handleChange} placeholder="e.g. Trade Subject, Mathematics, Science" icon={FiBook} />
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {step === 3 && (
              <motion.div key="step3" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-emerald-100 rounded-lg text-emerald-600"><MdSchool className="w-5 h-5" /></div>
                  Institute Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                  <div className="sm:col-span-2 lg:col-span-3"><InputField label="Institute Name" id="schoolName" value={formData.schoolName} onChange={handleChange} required icon={MdSchool} error={errors.schoolName} /></div>
                  <div>
                    <SelectField label="School Type" id="schoolType" value={formData.schoolType} onChange={handleChange} required icon={BsBuilding} error={errors.schoolType}
          ...
          <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 mt-8 bg-white/60 p-4 sm:p-6 rounded-2xl border border-emerald-100 shadow-sm backdrop-blur-md">
            <button 
              type="button"
              onClick={step === 0 ? handleBackNavigation : handleBack}
              className={`w-full sm:w-auto px-6 py-3 rounded-xl font-bold transition-colors focus:ring-4 flex items-center justify-center gap-2 text-emerald-700 bg-white border-2 border-emerald-200 hover:bg-emerald-50 focus:ring-emerald-100`}
            >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            ...
            {step < STEPS.length - 1 ? (
              <button 
                type="button"
                onClick={handleNext}
                className="w-full sm:w-auto px-8 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 shadow-lg shadow-emerald-300/50 transition-all hover:-translate-y-0.5 focus:ring-4 focus:ring-emerald-200 flex items-center justify-center gap-2"
              >
                Next Step
