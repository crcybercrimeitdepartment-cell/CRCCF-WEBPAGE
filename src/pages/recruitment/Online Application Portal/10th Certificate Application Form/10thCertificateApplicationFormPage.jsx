...
const inputClasses = "w-full bg-white border border-blue-200 rounded-xl px-4 py-3 text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-sm placeholder-gray-400 text-gray-700";
const labelClasses = "block text-xs font-bold text-blue-800 uppercase tracking-wider mb-2 ml-1";
const sectionClasses = "bg-white/60 backdrop-blur-md rounded-2xl p-5 sm:p-6 md:p-8 shadow-sm border border-blue-100";
const sectionTitleClasses = "text-lg sm:text-xl font-bold text-blue-900 mb-6 flex items-center gap-3 border-b border-blue-200 pb-4";
  ...
  <div>
    ...
    <div className="relative">
      <input
        ...
        className={`${inputClasses} ${error ? 'border-blue-400 ring-2 ring-blue-100' : ''} ${Icon ? 'pl-11' : ''}`}
      />
      {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />}
    </div>
    {error && <p className="text-blue-500 text-xs mt-1 flex items-center gap-1 font-semibold">⚠ {error}</p>}
  </div>
);
  ...
  <div>
    ...
    <div className="relative">
      <textarea
        ...
        placeholder={placeholder || `Enter ${label}`}
        className={`${inputClasses} resize-none ${error ? 'border-blue-400 ring-2 ring-blue-100' : ''} ${Icon ? 'pl-11 pt-4' : ''}`}
      />
    ...
    <div>
      ...
      <div className="relative">
        ...
        <div 
          ...
          className={`w-full bg-white border ${isOpen ? 'border-purple-500 ring-4 ring-purple-50' : 'border-blue-200'} rounded-xl px-4 py-3 text-base sm:text-sm transition-all shadow-sm flex justify-between items-center cursor-pointer hover:border-purple-300 ${error ? 'border-blue-400 ring-2 ring-blue-100' : ''} ${Icon ? 'pl-11' : ''}`}
        >
          {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />}
          ...
          <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
            <FiChevronDown className={`w-4 h-4 ${isOpen ? 'text-blue-500' : 'text-gray-400'}`} />
          </motion.div>
        </div>
          ...
          {isOpen && (
            <>
              ...
              <motion.div
                ...
                className="absolute z-50 w-full mt-2 bg-white border border-blue-100 rounded-xl shadow-[0_10px_40px_-10px_rgba(59,130,246,0.2)] overflow-hidden origin-top"
              >
                <div className="max-h-60 overflow-y-auto py-1 custom-scrollbar">
                  {options.map((option) => (
                    <div
                      ...
                      className={`px-4 py-3 sm:py-2.5 text-base sm:text-sm cursor-pointer transition-colors flex items-center justify-between ${value === option.value ? 'bg-blue-50 text-blue-700 font-bold' : 'text-gray-700 hover:bg-blue-50/50'}`}
                    >
                      {option.label}
                      {value === option.value && <FiCheckCircle className="w-4 h-4 text-blue-500" />}
                    </div>
                  ))}
                </div>
              </motion.div>
...
function TenthCertificateForm({ onBack }) {
  ...
  if (isPreview) {
    return (
      <div className="min-h-screen bg-[#f0f9ff] relative overflow-hidden font-sans pb-20 pt-28 sm:pt-32">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="bg-white/95 backdrop-blur-md rounded-3xl p-6 sm:p-10 shadow-xl border border-blue-100">
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50 text-blue-700 text-xs font-bold mb-3 border border-blue-200">
            ...
            <div className="bg-slate-50/80 rounded-2xl p-6 border border-slate-200 mb-8 space-y-6">
              <div>
                <h3 className="text-xs font-black uppercase tracking-wider text-blue-800 mb-3 pb-2 border-b border-slate-200">
            ...
            <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-200">
              ...
              <button
                ...
                className="w-full sm:w-auto px-8 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-300/50 transition-all hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer"
              >
    ...
    return (
      ...
      >
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center border border-blue-100">
          ...
          <button onClick={handleReset} className="mt-8 text-purple-600 font-bold hover:underline">Submit Another Application</button>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f0f9ff] relative overflow-hidden font-sans pb-20">
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-blue-200/50 rounded-full mix-blend-multiply filter blur-[80px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-indigo-200/50 rounded-full mix-blend-multiply filter blur-[80px] pointer-events-none" />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-28 sm:pt-32 pb-20 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex flex-col items-center text-center">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-5 mb-4">
            <div className="inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 shadow-lg shadow-blue-300/50 text-white shrink-0">
            ...
            <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight text-center sm:text-left">
              10th Certificate <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Application Form</span>
            </h1>
          </div>
          <p className="text-gray-600 max-w-2xl text-base sm:text-lg mx-auto">
            Please fill out all the required details carefully to request your 10th standard academic certificate.
          </p>
        </motion.div>
        ...
        <div className="flex justify-end max-w-3xl mx-auto mb-3 px-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-blue-300 bg-blue-50/90 text-blue-900 hover:bg-blue-100 font-bold text-xs shadow-xs transition-all cursor-pointer"
          >
            <FiFastForward className="w-3.5 h-3.5 text-blue-600" />
            <span>Skip to Preview</span>
          </button>
        </div>
<div className="mb-12 max-w-3xl mx-auto px-4 sm:px-0">
          <div className="flex items-center justify-between relative">
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1.5 bg-blue-100 -z-10 rounded-full"></div>
            <div 
              className="absolute left-0 top-1/2 -translate-y-1/2 h-1.5 bg-gradient-to-r from-blue-400 to-indigo-600 -z-10 rounded-full transition-all duration-500 ease-in-out"
            ...
            {STEPS.map((stepData, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center">
                  <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all duration-500 border-4 ${isActive ? 'bg-purple-600 border-blue-100 text-white shadow-lg shadow-purple-300' : 'bg-white border-blue-100 text-purple-300'}`}>
                    <StepIcon className="w-4 h-4 sm:w-6 sm:h-6" />
                  </div>
                  <span className={`mt-3 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-center ${isActive ? 'text-blue-800' : 'text-purple-400'} hidden sm:block`}>{stepData.label}</span>
                </div>
              )
            })}
          </div>
          <div className="text-center mt-4 sm:hidden">
             <span className="text-sm font-bold text-blue-800 uppercase tracking-wider">{STEPS[step].label}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="relative">
            {step === 0 && (
              <motion.div key="step0" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-blue-100 rounded-lg text-purple-600"><FiUser className="w-5 h-5" /></div>
                  Personal Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                  ...
                  <div className="sm:col-span-2 lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                    ...
                    <div>
                      ...
                      <label className="flex items-center gap-1.5 cursor-pointer text-[10px] sm:text-xs font-bold text-blue-700 hover:text-blue-800 transition-colors uppercase tracking-wider mt-2">
                        <input type="checkbox" checked={sameAddress} onChange={handleSameAddress} className="w-4 h-4 text-purple-600 rounded border-purple-300 focus:ring-blue-500 cursor-pointer accent-purple-600" />
            ...
            {step === 1 && (
              <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-blue-100 rounded-lg text-purple-600"><FaUniversity className="w-5 h-5" /></div>
                  Institution Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            ...
            {step === 2 && (
              <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-blue-100 rounded-lg text-purple-600"><FiBarChart2 className="w-5 h-5" /></div>
                  Academic Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            ...
            {step === 3 && (
              <motion.div key="step3" variants={stepVariants} initial="hidden" animate="visible" className={sectionClasses}>
                <h2 className={sectionTitleClasses}>
                  <div className="p-2 bg-blue-100 rounded-lg text-purple-600"><HiOutlineAcademicCap className="w-5 h-5" /></div>
                  Certificate Details
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          ...
          <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 mt-8 bg-white/60 p-4 sm:p-6 rounded-2xl border border-blue-100 shadow-sm backdrop-blur-md">
            <button 
              type="button"
              onClick={step === 0 ? handleBackNavigation : handleBack}
              className={`w-full sm:w-auto px-6 py-3 rounded-xl font-bold transition-colors focus:ring-4 flex items-center justify-center gap-2 text-blue-700 bg-white border-2 border-blue-200 hover:bg-blue-50 focus:ring-blue-100`}
            >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            ...
            {step < STEPS.length - 1 ? (
              <button 
                type="button"
                onClick={handleNext}
                className="w-full sm:w-auto px-8 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-300/50 transition-all hover:-translate-y-0.5 focus:ring-4 focus:ring-blue-200 flex items-center justify-center gap-2"
              >
                Next Step
...
export default TenthCertificateForm;
