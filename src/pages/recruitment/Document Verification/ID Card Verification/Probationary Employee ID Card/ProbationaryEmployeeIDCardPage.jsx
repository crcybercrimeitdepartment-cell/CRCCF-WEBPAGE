...
import { 
    User, Phone, Mail, Briefcase, Hash, CheckCircle, ArrowLeft, ShieldAlert, Building, FileText, IdCard, Calendar, MapPin, Award, UserCheck
} from 'lucide-react';
import { idCardDetails } from './idCardData';
import { InfoField, PersonNameFields } from '../utils/nameHelper';

export default function ProbationaryEmployeeIDCardPage({ themeColor = '#d35400' }) {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] py-8 pb-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex flex-col">
            ...
            <div className="max-w-4xl w-full mx-auto mb-8 relative z-10 text-center">
                <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0C1A3A] tracking-tight">
                    Probationary Employee ID Card Verification
                </h1>
                {!isVerified && <p className="text-slate-500 text-sm font-medium mt-1">Enter details to verify official ID credential</p>}
                    ...
                    >
                        <div className="bg-white p-8 sm:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] w-full max-w-lg border border-slate-100">
                            <div className="text-center mb-8">
                                ...
                                <h2 className="text-xl font-bold text-slate-800">Lookup Probationary Employee ID Card</h2>
                            ...
                            <form onSubmit={handleVerify} className="space-y-5">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">ID Card Number / Staff ID</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                        ...
                                        <input
                                            ...
                                            placeholder="e.g. IDC-2024-PRB09"
                                            required
                                        />
                                    </div>
                                </div>
                                ...
                                <button
                                    type="submit"
                                    className="w-full py-3.5 px-4 text-white rounded-xl font-bold shadow-lg transition-all flex items-center justify-center mt-6 hover:brightness-110 cursor-pointer"
                    ...
                    >
                        ...
                        <div className="bg-white/60 backdrop-blur-xl border-b border-slate-100/80 p-8 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative z-10">
                            <div className="flex items-center space-x-6">
                                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl flex items-center justify-center shadow-sm ring-2 ring-white overflow-hidden bg-slate-100" style={{ borderColor: themeColor }}>
                                    {idCardDetails.idCardPhoto ? (
                                        <img src={idCardDetails.idCardPhoto} alt={idCardDetails.name} className="w-full h-full object-cover" />
                                    ) : (
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 font-bold text-sm sm:text-base flex items-center">
                                        ...
                                        {idCardDetails.designation} • {idCardDetails.department}
                                    </p>
                                </div>
                            </div>
                            <button 
                                onClick={() => setIsVerified(false)}
                                className="px-6 py-3 font-bold rounded-xl transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5 border cursor-pointer"
