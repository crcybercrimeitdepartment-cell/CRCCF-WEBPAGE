...
import { 
    User, Calendar, Briefcase, Hash, CheckCircle, 
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] py-8 pb-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex flex-col">
            ...
            <div className="max-w-4xl w-full mx-auto mb-8 relative z-10 text-center">
                ...
                {!isVerified && <p className="text-slate-500 text-sm font-medium mt-1">Enter agreement details to verify</p>}
                ...
                {!isVerified ? (
                    ...
                    >
                        <div className="bg-white p-8 sm:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] w-full max-w-lg border border-slate-100">
                            ...
                            <form onSubmit={handleVerify} className="space-y-5">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Agreement Registration No</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                ...
                                >
                                    Verify Agreement <ArrowLeft className="ml-2 rotate-180" size={18} />
                                </button>
                            </form>
                        </div>
                    </motion.div>
                    ...
                    >
                        ...
                        <div className="bg-white/60 backdrop-blur-xl border-b border-slate-100/80 p-8 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative z-10">
                            <div className="flex items-center space-x-6">
                                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl flex items-center justify-center shadow-sm ring-2 ring-white" style={{ backgroundColor: `${themeColor}15`, borderColor: themeColor }}>
                                    <FileText size={40} style={{ color: themeColor }} />
                                </div>
                                <div>
                                    <div className="flex flex-wrap items-center gap-4 mb-2">
                                        <h2 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight">{agreementDetails.agreementTitle}</h2>
                                        <span 
                                    ...
                                    <p className="text-slate-500 font-bold text-sm sm:text-base flex items-center">
                                        <Hash size={18} className="mr-2" style={{ color: themeColor }} />
                                        Registration No: {agreementDetails.registrationNumber}
                                    </p>
                                </div>
                            </div>
                            <button 
                        ...
                        <div className="p-8 sm:p-12 grid grid-cols-1 md:grid-cols-2 gap-10 bg-slate-50/30 relative z-10">
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-extrabold uppercase tracking-widest flex items-center" style={{ color: themeColor }}>
                                    <FileText size={18} className="mr-2.5" /> Agreement Specifications
                                </h3>
                                <div className="grid grid-cols-1 gap-4">
                                    <InfoField icon={Link} label="Agreement Purpose" value={agreementDetails.agreementPurpose} themeColor={themeColor} />
                                    <InfoField icon={Briefcase} label="Agreement Type" value={agreementDetails.agreementType} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Agreement Activity" value={agreementDetails.agreementActivity} themeColor={themeColor} />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                                    <InfoField icon={Calendar} label="Validity" value={agreementDetails.validity} themeColor={themeColor} />
                                    <InfoField icon={Calendar} label="Expiry Date" value={agreementDetails.expireDate} themeColor={themeColor} />
                                </div>
                            </div>
                        </div>
                    </motion.div>
