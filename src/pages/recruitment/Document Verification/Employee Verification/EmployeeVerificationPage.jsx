    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] py-8 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex flex-col">
            ...
            <div className="max-w-4xl w-full mx-auto mb-8 relative z-10 text-center">
                ...
                {!isVerified && <p className="text-slate-500 text-sm font-medium mt-1">Enter employee details to verify</p>}
                    ...
                    >
                        <div className="bg-white p-8 sm:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] w-full max-w-md border border-slate-100">
                            ...
                            <form onSubmit={handleVerify} className="space-y-5">
                                ...
                                >
                                    Verify Employee <ArrowLeft className="ml-2 rotate-180" size={18} />
                                </button>
                            </form>
                        </div>
                    </motion.div>
                    ...
                    >
                        ...
                        <div className="bg-white/60 backdrop-blur-xl border-b border-slate-100/80 p-8 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative z-10">
                            <div className="flex items-center space-x-6">
                                ...
                                <img src={employeeDetails.photo} alt={employeeDetails.name} className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl object-cover shadow-sm ring-2 ring-white" style={{ borderColor: themeColor }} />
                                <div>
                                    <div className="flex flex-wrap items-center gap-4 mb-2">
                                        <h2 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight">{employeeDetails.name}</h2>
                                        <span 
                                    ...
                                    <p className="text-slate-500 font-bold text-sm sm:text-base flex items-center">
                                        <Briefcase size={18} className="mr-2" style={{ color: themeColor }} />
                        ...
                        <div className="p-8 sm:p-12 grid grid-cols-1 md:grid-cols-2 gap-10 bg-slate-50/30 relative z-10">
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-extrabold uppercase tracking-widest flex items-center" style={{ color: themeColor }}>
                                    <Briefcase size={18} className="mr-2.5" /> Employment Details
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InfoField icon={Hash} label="Employee ID" value={employeeDetails.employeeId} themeColor={themeColor} />
                                    <InfoField icon={Building} label="Department" value={employeeDetails.department} themeColor={themeColor} />
                                    <InfoField icon={Tag} label="Category" value={employeeDetails.employeeCategory} themeColor={themeColor} />
                                    <InfoField icon={Users} label="Employee Type" value={employeeDetails.employeeType} themeColor={themeColor} />
                                    <InfoField icon={Clock} label="Duty Mode" value={employeeDetails.dutyMode} themeColor={themeColor} />
                                    <InfoField icon={CalendarClock} label="Joining Date" value={employeeDetails.joiningDate} themeColor={themeColor} />
                                </div>
                            </div>
                        </div>
                    </motion.div>
