...
import { 
    User, Phone, Mail, Briefcase, Hash, CheckCircle, ArrowLeft, Search, ShieldAlert, Building, FileText, AtSign, UserCheck, FileSearch
} from 'lucide-react';
import { documentDetails } from './documentData';
...
export default function DocumentVerificationPage({ onBack, themeColor = '#3b82f6' }) {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] py-8 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex flex-col">
            ...
            <div className="max-w-4xl w-full mx-auto mb-8 relative z-10 text-center">
                <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0C1A3A] tracking-tight">Document Verification</h1>
                        ...
                        <div className="bg-white p-8 sm:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] w-full max-w-md border border-slate-100">
                            <div className="text-center mb-8">
                                <div className="mx-auto w-16 h-16 rounded-2xl flex items-center justify-center mb-4 shadow-inner" style={{ color: themeColor, backgroundColor: `${themeColor}15` }}>
                                    <FileSearch size={28} strokeWidth={2} />
                                </div>
                            ...
                            <form onSubmit={handleVerify} className="space-y-5">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Document Number</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                            <FileText size={18} className="text-slate-400" />
                                        </div>
                    ...
                    >
                        ...
                        <div className="bg-white/60 backdrop-blur-xl border-b border-slate-100/80 p-8 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative z-10">
                            <div className="flex items-center space-x-6">
                                <img src={documentDetails.documentPhotoPreview} alt="Preview" className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl object-cover shadow-sm ring-2 ring-white" style={{ borderColor: themeColor }} />
                                <div>
                                    <div className="flex flex-wrap items-center gap-4 mb-2">
                                        <h2 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight">Document Details</h2>
                                        <span 
                        ...
                        <div className="p-8 sm:p-12 bg-slate-50/30 relative z-10">
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <InfoField icon={FileText} label="Sl No" value={documentDetails.slNo} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Document Name" value={documentDetails.documentName} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Document Type" value={documentDetails.documentType} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Document Number" value={documentDetails.documentNumber} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Document Category" value={documentDetails.documentCategory} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Document Purpose" value={documentDetails.documentPurpose} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Issuing Organization" value={documentDetails.issuingOrganization} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Issuing Authority" value={documentDetails.issuingAuthority} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Issue Date" value={documentDetails.issueDate} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Valid From" value={documentDetails.validFrom} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Expiry Date" value={documentDetails.expiryDate} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Holder / Applicant Name" value={documentDetails.holderApplicantName} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Father's Name" value={documentDetails.fatherSName} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Date of Birth" value={documentDetails.dateOfBirth} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Address" value={documentDetails.address} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Mobile Number" value={documentDetails.mobileNumber} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Email ID" value={documentDetails.emailId} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Registration Number" value={documentDetails.registrationNumber} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Document Status" value={documentDetails.documentStatus} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Verification Date" value={documentDetails.verificationDate} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Verified By" value={documentDetails.verifiedBy} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Remarks" value={documentDetails.remarks} themeColor={themeColor} />
