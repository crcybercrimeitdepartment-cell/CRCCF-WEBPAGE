...
import { 
    User, Phone, Mail, Briefcase, Hash, CheckCircle, ArrowLeft, Search, ShieldAlert, Building, FileText, AtSign, UserCheck, AlertCircle, Calendar, Award, MapPin
} from 'lucide-react';
import { noticeDetails } from './noticeData';
import { InfoField, PersonNameFields } from '../utils/nameHelper';

export default function NoticeVerificationPage({ onBack, themeColor = '#f59e0b' }) {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] py-8 pb-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex flex-col">
            ...
            <div className="max-w-4xl w-full mx-auto mb-8 relative z-10 text-center">
                <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0C1A3A] tracking-tight">Notice Verification</h1>
                    ...
                    >
                        <div className="bg-white p-8 sm:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] w-full max-w-lg border border-slate-100">
                            <div className="text-center mb-8">
                                ...
                                <h2 className="text-xl font-bold text-slate-800">Lookup Notice</h2>
                                <p className="text-sm text-slate-500 mt-2">Enter signatory name and notice registration ID to retrieve the verification record.</p>
                            </div>
                            ...
                            <div className="flex items-center space-x-6">
                                ...
                                <div>
                                    <div className="flex flex-wrap items-center gap-4 mb-2">
                                        <h2 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight">{noticeDetails.noticeTitle}</h2>
                                        <span 
                                    ...
                                    <p className="text-slate-500 font-bold text-sm sm:text-base flex items-center">
                                        ...
                                        {noticeDetails.noticeType} • ID: {noticeDetails.noticeRegistrationId}
                                    </p>
                                </div>
                            </div>
                            <button 
                        ...
                        <div className="p-8 sm:p-12 bg-slate-50/30 relative z-10">
                            <div className="space-y-6">
                                <h3 className="text-sm font-extrabold uppercase tracking-widest flex items-center" style={{ color: themeColor }}>
                                    <FileText size={18} className="mr-2.5" /> Notice Record & Authority Details
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <InfoField icon={Hash} label="Sl No" value={noticeDetails.slNo} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Notice Number" value={noticeDetails.noticeNumber} themeColor={themeColor} />
                                    <InfoField icon={Hash} label="Notice Registration ID" value={noticeDetails.noticeRegistrationId || noticeDetails.referenceNumber} themeColor={themeColor} />
                                    ...
                                    <InfoField icon={Calendar} label="Notice Date" value={noticeDetails.noticeDate} themeColor={themeColor} />
                                    <InfoField icon={Calendar} label="Effective Date" value={noticeDetails.effectiveDate} themeColor={themeColor} />
                                    <InfoField icon={Calendar} label="Expiry Date" value={noticeDetails.expiryDate} themeColor={themeColor} />
                                    <InfoField icon={Building} label="Issuing Organization" value={noticeDetails.issuingOrganization} themeColor={themeColor} />
                                    <InfoField icon={Building} label="Issuing Department" value={noticeDetails.issuingDepartment} themeColor={themeColor} />
                                    <InfoField icon={Award} label="Issuing Authority" value={noticeDetails.issuingAuthority} themeColor={themeColor} />
                                    <InfoField icon={FileText} label="Notice Purpose" value={noticeDetails.noticePurpose} themeColor={themeColor} />
                                    <InfoField icon={UserCheck} label="Applicable To" value={noticeDetails.applicableTo} themeColor={themeColor} />
                                    <PersonNameFields 
                                        icon={User} 
                                        baseLabel="Authorized Signatory" 
                                        fullName={noticeDetails.authorizedSignatory}
                                        firstName={noticeDetails.authorizedSignatoryFirstName}
                                        middleName={noticeDetails.authorizedSignatoryMiddleName}
                                        lastName={noticeDetails.authorizedSignatoryLastName}
                                        themeColor={themeColor} 
                                    />
                                    <InfoField icon={Phone} label="Contact Number" value={noticeDetails.contactNumber} themeColor={themeColor} />
                                    <InfoField icon={Mail} label="Email ID" value={noticeDetails.emailId} themeColor={themeColor} />
                                    <InfoField icon={MapPin} label="Address" value={noticeDetails.address} themeColor={themeColor} />
                                    <InfoField icon={CheckCircle} label="Notice Status" value={noticeDetails.noticeStatus} themeColor={themeColor} />
                                    <InfoField icon={Calendar} label="Verification Date" value={noticeDetails.verificationDate} themeColor={themeColor} />
                                    <InfoField icon={Award} label="Verified By" value={noticeDetails.verifiedBy} themeColor={themeColor} />
