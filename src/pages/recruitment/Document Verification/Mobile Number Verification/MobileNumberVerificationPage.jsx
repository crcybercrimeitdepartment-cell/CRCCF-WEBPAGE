...
import { 
    ...
    Building, FileText, Smartphone, UserCheck
} from 'lucide-react';
import { mobileDetails } from './mobileData';
import { InfoField, PersonNameFields } from '../utils/nameHelper';

export default function MobileNumberVerificationPage({ onBack, themeColor = '#f97316' }) {
    ...
    const handleVerify = (e) => {
        ...
        const cleanInput = mobileInput.replace(/[^0-9+]/g, '');
        
        if (cleanInput === '+918249585134' || cleanInput === '8249585134') {
            setIsVerified(true);
        } else {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] py-8 pb-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex flex-col">
            ...
            <div className="max-w-4xl w-full mx-auto mb-8 relative z-10 text-center">
                <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0C1A3A] tracking-tight">Mobile Number Verification</h1>
                {!isVerified && <p className="text-slate-500 text-sm font-medium mt-1">Enter mobile number to verify assignment details</p>}
                ...
                {!isVerified ? (
                    ...
                    >
                        <div className="bg-white p-8 sm:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] w-full max-w-md border border-slate-100">
                            <div className="text-center mb-8">
                                ...
                                <h2 className="text-xl font-bold text-slate-800">Lookup Mobile Number</h2>
                                <p className="text-sm text-slate-500 mt-2">Enter the 10-digit or country-coded mobile number to retrieve assignment details.</p>
                            </div>
                            ...
                            <form onSubmit={handleVerify} className="space-y-5">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            style={ focusedInput === 'mobile' ? { borderColor: themeColor, boxShadow: `0 0 0 2px ${themeColor}33`, backgroundColor: 'white' } : {} }
                                            placeholder="e.g. 8249585134"
                                            required
                                        />
                                    </div>
                                </div>
                                ...
                                >
                                    Verify Mobile <ArrowLeft className="ml-2 rotate-180" size={18} />
                                </button>
                            </form>
                        </div>
                    </motion.div>
                    ...
                    >
                        ...
                        <div className="bg-white/60 backdrop-blur-xl border-b border-slate-100/80 p-8 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative z-10">
                            <div className="flex items-center space-x-6">
                                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl flex items-center justify-center shadow-sm ring-2 ring-white" style={{ backgroundColor: `${themeColor}15`, borderColor: themeColor }}>
                                    <Smartphone size={40} style={{ color: themeColor }} />
                                </div>
                                <div>
                                    <div className="flex flex-wrap items-center gap-4 mb-2">
                                        <h2 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight">{mobileDetails.mobileNo}</h2>
                                        <span 
                                    ...
                                    <p className="text-slate-500 font-bold text-sm sm:text-base flex items-center">
                                        ...
                                        Assigned to: {mobileDetails.name}
                                    </p>
                                </div>
                            </div>
                            <button 
                        ...
                        <div className="p-8 sm:p-12 grid grid-cols-1 md:grid-cols-2 gap-10 bg-slate-50/30 relative z-10">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <PersonNameFields 
                                        ...
                                        fullName={mobileDetails.name}
                                        firstName={mobileDetails.firstName}
                                        middleName={mobileDetails.middleName}
                                        lastName={mobileDetails.lastName}
                                        themeColor={themeColor} 
                                    />
                                    <InfoField icon={Hash} label="Employee Sl No." value={mobileDetails.employeeSlNo} themeColor={themeColor} />
                                    <InfoField icon={Briefcase} label="Designation" value={mobileDetails.designation} themeColor={themeColor} />
                                    <InfoField icon={Building} label="Department" value={mobileDetails.department} themeColor={themeColor} />
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-extrabold uppercase tracking-widest flex items-center" style={{ color: themeColor }}>
                                    <Phone size={18} className="mr-2.5" /> Mobile Specifications
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <InfoField icon={Phone} label="Registered Mobile No." value={mobileDetails.mobileNo} themeColor={themeColor} />
                                    <InfoField icon={Mail} label="Registered Mail ID" value={mobileDetails.mailId} themeColor={themeColor} />
                                    <InfoField icon={CheckCircle} label="Mobile Status" value={mobileDetails.mobileStatus} themeColor={themeColor} />
                                </div>
                                <div className="mt-4">
                                    <InfoField icon={FileText} label="Remark" value={mobileDetails.remark} themeColor={themeColor} />
                                </div>
                            </div>
                        </div>
                    </motion.div>
