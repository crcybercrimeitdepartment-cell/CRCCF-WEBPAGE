...
export default function BuildingAnimation({ onComplete }) {
  ...
  const playDoorbellSound = () => {
    try {
      const audio = new Audio('/door_bell.mpeg');
  ...
  const playDoorOpeningSound = () => {
    try {
      const audio = new Audio('/door_opening_sound.mp3');
      audio.play().catch(e => console.warn("Audio playback failed:", e));
    } catch (e) {
      console.warn('Audio API not supported', e);
    }
    ...
    if (!isDoorOpen && !isWalkingBack && !isReceptionistForward && !isWebsiteVisible) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          // Greet for 4s, then go back inside
          setTimeout(() => {
            setHasReachedDoor(false);
            setIsReceptionistForward(false);
            setIsWalkingBack(true);
            
            // Trigger website fly-out suddenly as she starts walking back!
            setTimeout(() => {
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-b from-sky-400 via-blue-200 to-blue-100 flex flex-col relative overflow-hidden items-center justify-end">
      ...
      <div className={`fixed inset-0 z-[1000] bg-white transition-all duration-[1400ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] flex flex-col transform origin-[50%_85%] ${isWebsiteVisible ? 'scale-100 opacity-100 rotate-0 blur-none pointer-events-auto' : 'scale-[0.05] opacity-0 -rotate-6 blur-md pointer-events-none'}`}>
          ...
          <div className="w-full bg-[#0f172a] h-12 flex items-center justify-between px-6 shadow-md z-10 shrink-0">
             <div className="flex items-center gap-4">
                ...
                <div className="bg-[#1e293b] px-4 py-1.5 rounded-md text-xs text-gray-400 font-mono flex items-center gap-2 border border-[#334155] shadow-inner hidden sm:flex">
                   <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4"></path></svg>
                   crccf-official-website.com
                </div>
             </div>
             <button onClick={closeWebsite} className="bg-red-500 hover:bg-red-600 px-5 py-1.5 rounded-md text-white font-bold transition-colors text-sm shadow-lg flex items-center gap-2 cursor-pointer pointer-events-auto">
          ...
          <div className="relative flex-1 w-full bg-slate-50 flex flex-col items-center justify-center">
             <div className="animate-pulse flex flex-col items-center">
                ...
                <p className="text-gray-500 font-medium">Entering CRCCF Official Website...</p>
             </div>
          </div>
      </div>
